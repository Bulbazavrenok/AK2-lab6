...
MODULE_AUTHOR("Uliana Kucherenko, IO-05");
...
MODULE_PARM_DESC(num_of_greet, "'Hello, World!' line will be typed n-number of times");
...
static int __init hello_init(void)
{
    if (num_of_greet == 0 || (num_of_greet >= 5 && num_of_greet <= 10)) {
        printk(KERN_WARNING "is 0 or between 5 and 10");  
    }

    if (num_of_greet > 10) {
        printk(KERN_ERR "is too large");
        return -EINVAL;
    }
